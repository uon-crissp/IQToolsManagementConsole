...
namespace DataLayer
{
    public class Entity : ProcessBase
    {
        ...
        private object MsSQLObject(string ConString, Hashtable Params, string CommandText, ClsUtility.ObjectEnum Obj)
        {
            ...
            { if (CommandText.Substring(0, 28).ToUpper() == "DROP TABLE TMP_PATIENTMASTER") theCmd.CommandType = CommandType.Text; }
            if (CommandText.Length >= 30)
            { if (CommandText.Substring(0, 14).ToUpper() == "OPEN SYMMETRIC") theCmd.CommandType = CommandType.Text; }
            if (CommandText.Length >= 15)
            { if (CommandText.Substring(0, 15).ToUpper() == "CLOSE SYMMETRIC") theCmd.CommandType = CommandType.Text; }
        ...
        public static void RecreateDB(string dbPath)
        {
            try
            {
